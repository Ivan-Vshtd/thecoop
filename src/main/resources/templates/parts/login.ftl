...
<form action="${path}" method="post" xmlns="http://www.w3.org/1999/html">  <#--https://getbootstrap.com-->
    ...
    <div>
        <button class="btn btn-outline-secondary" type="submit"><#if isRegisterForm>Add<#else>Sign in</#if></button>
    </div>
    <#if !isRegisterForm><a href="/registration">Register</a><#else><a href="/login">Sign in
    </#if>
